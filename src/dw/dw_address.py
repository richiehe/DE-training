...
if __name__ == '__main__':
    source_dbtable = "ods_address"
    target_dbtable = "dw.dw_address"

    spark = init_spark_session(target_dbtable)

    df = spark_read_from_db(spark, source_dbtable, col('processed_date') == '2022-09-19')

    df = df.dropDuplicates(['address_id'])

    if df.count() > 0:
        remove_partition_data(df, 'dw', 'address', DATASET_SCHEMA.get('address').get('partition_field'))
